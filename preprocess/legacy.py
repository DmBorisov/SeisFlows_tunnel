...
class legacy(object):
    ...
    def prepare_eval_grad(self, path='.'):
        ...
        #if PAR.SPIKE_STF:
        #    s = self.multichannel(self.convolve_stf, [s], [h])
    ...
    # added by DmBorisov
    def convolve_stf(self, s, h):
        """ Performs convolution of synthetic with source time function
        """
        file = PATH.SOURCE_WAVELET + '/' + PAR.STF_NAME

        stf = np.fromfile(file, dtype='float32', count=-1)
        for i in range(h.nr):
            s[:,i] = np.convolve(s[:,i], stf, 'same')
