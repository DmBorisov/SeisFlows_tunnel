...
class base(object):
    ...
    # DB: modified and moved from solver/specfem3d_workaround
    def load_xyz(self, *args, **kwargs):
        """ reads SPECFEM model or kernel
          Models are stored in Fortran binary format and separated into multiple
          files according to material parameter and processor rank.
        """
        model = self.load(*args, **kwargs)
        model_path = args[0]
    ...
    def export_model(self, path, solver_parameters=['rho', 'vp', 'vs']):
        if self.getnode == 0:
            ...
            # DB: copy mesh coordinate files, if they exist 
            for key in ['x_loc', 'y_loc', 'z_loc']:
                try:
                    files = glob(join(self.model_databases, '*'+key+'.bin'))
                    unix.cp(files, path)
                except:
